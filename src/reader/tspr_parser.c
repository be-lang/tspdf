...
static inline bool keyword_at(const uint8_t *data, size_t len, size_t pos, const char *s) {
    size_t slen = strlen(s);
    if (!data || !s || pos + slen > len) return false;
    if (memcmp(data + pos, s, slen) != 0) return false;
    return pos + slen == len || is_ws_or_delim(data[pos + slen]);
}

static inline bool match_keyword(TspdfParser *p, const char *s) {
    size_t saved = p->pos;
    if (!match(p, s)) return false;
    if (!at_end(p) && !is_ws_or_delim(peek(p))) {
        p->pos = saved;
        return false;
    }
    return true;
...
static bool parse_i64_token(const char *buf, int64_t *out) {
    if (!buf || !out || buf[0] == '\0') {
        return false;
    }
...
static bool parse_u64_token(const char *buf, uint64_t *out) {
    if (!buf || !out || buf[0] == '\0') {
        return false;
    }
...
static bool parse_u32_token(const char *buf, uint32_t *out) {
    uint64_t value = 0;
    if (!parse_u64_token(buf, &value) || value > UINT32_MAX) {
        return false;
    }
...
static bool parse_u16_token(const char *buf, uint16_t *out) {
    uint64_t value = 0;
    if (!parse_u64_token(buf, &value) || value > UINT16_MAX) {
        return false;
    }
...
static TspdfObj *tspdf_parse_array_body(TspdfParser *p) {
    p->pos++; // skip '['
...
// Bound recursion: a hostile PDF with deeply nested arrays/dicts would
// otherwise overflow the C stack (verified stack-overflow DoS). The counter
// wrapper guarantees the depth is decremented on every exit path.
static TspdfObj *tspdf_parse_array(TspdfParser *p) {
    if (p->depth >= TSPDF_MAX_PARSE_DEPTH) {
        p->error = TSPDF_ERR_PARSE;
        return NULL;
    }
    p->depth++;
    TspdfObj *obj = tspdf_parse_array_body(p);
    p->depth--;
    return obj;
}
...
static TspdfObj *tspdf_parse_dict_body(TspdfParser *p) {
...
// Bound recursion (see tspdf_parse_array): the wrapper guarantees the depth
...
static TspdfObj *tspdf_parse_dict(TspdfParser *p) {
    ...
    p->depth++;
    TspdfObj *obj = tspdf_parse_dict_body(p);
    p->depth--;
    return obj;
}
...
static TspdfObj *tspdf_parse_number(TspdfParser *p) {
    ...
    if (has_dot) {
        errno = 0;
        char *end = NULL;
        double real_val = strtod(numbuf, &end);
        if (errno == ERANGE || !end || *end != '\0') {
            p->error = TSPDF_ERR_PARSE;
            return NULL;
        }
    ...
    int64_t int_val = 0;
    if (!parse_i64_token(numbuf, &int_val)) {
        p->error = TSPDF_ERR_PARSE;
        return NULL;
    }
    ...
    if (int_val >= 0 && p->data[start] != '+') {
        ...
        if (!at_end(p) && isdigit(peek(p))) {
            ...
            if (glen < sizeof(genbuf)) {
                ...
                genbuf[glen] = '\0';
                gen_valid = parse_u16_token(genbuf, &gen_val);

                tspdf_skip_whitespace(p);

                if (match_keyword(p, "R")) {
                    if (!gen_valid || int_val > UINT32_MAX) {
                        p->error = TSPDF_ERR_PARSE;
                        return NULL;
                    }
                    TspdfObj *obj = alloc_obj(p);
...
TspdfObj *tspdf_parse_indirect_obj(TspdfParser *p, uint32_t *out_num, uint16_t *out_gen) {
    ...
    *out_gen = gen;
    ...
    if (inner->type == TSPDF_OBJ_DICT && !at_end(p) && match_keyword(p, "stream")) {
        ...
        size_t raw_len = 0;
        if (len_obj && len_obj->type == TSPDF_OBJ_INT && len_obj->integer >= 0) {
            raw_len = (size_t)len_obj->integer;
        }

        // Advance past stream data
        size_t expected_end = raw_offset;
        if (raw_offset <= p->len && raw_len <= p->len - raw_offset) {
            expected_end = raw_offset + raw_len;
            p->pos = expected_end;
        }

        // Try to find "endstream"
        tspdf_skip_whitespace(p);
        if (!match_keyword(p, "endstream")) {
            ...
            for (size_t i = raw_offset; i + needle_len <= p->len; i++) {
                if (keyword_at(p->data, p->len, i, needle)) {
        ...
        tspdf_skip_whitespace(p);
        match_keyword(p, "endobj"); // consume endobj

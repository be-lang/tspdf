...
static bool get_bool(TspdfObj *dict, const char *key, bool def) {
    TspdfObj *obj = tspdf_dict_get(dict, key);
    if (!obj || obj->type != TSPDF_OBJ_BOOL) return def;
    return obj->boolean;
}

/* ISO 32000-2 Algorithm 2.A: the 32-byte /UE and /OE strings are the file
 * encryption key wrapped with AES-256 in CBC mode, no padding, IV of all
 * zeroes. (An earlier version used ECB, which corrupts the second 16-byte
 * block and produces a wrong key.) */
static void aes256_cbc_zero_iv_decrypt32(const uint8_t key[32],
                                         const uint8_t in[32],
                                         uint8_t out[32]) {
    uint8_t zero_iv[16] = {0};
    Aes aes;
    aes_init(&aes, key, 256);
    aes_decrypt_cbc(&aes, zero_iv, in, out, 32);
}

static void aes256_cbc_zero_iv_encrypt32(const uint8_t key[32],
                                         const uint8_t in[32],
                                         uint8_t out[32]) {
    uint8_t zero_iv[16] = {0};
    Aes aes;
    aes_init(&aes, key, 256);
    aes_encrypt_cbc(&aes, zero_iv, in, out, 32);
}
...
static void compute_file_key(const char *password,
                             ...
                             int key_len, int R, bool encrypt_metadata,
                             uint8_t *out_key) {
    (void)O_len;
    uint8_t padded[32];
    ...
    /* Algorithm 2 step (f): if the document metadata is not encrypted and
     * R >= 4, feed 0xFFFFFFFF to the hash after the file ID. */
    if (!encrypt_metadata && R >= 4) {
        static const uint8_t sentinel[4] = {0xFF, 0xFF, 0xFF, 0xFF};
        md5_update(&md5, sentinel, 4);
    }
...
static bool verify_owner_password(TspdfCrypt *crypt,
    ...
    compute_file_key(NULL, O, 32, P, file_id, file_id_len,
                     crypt->key_len, crypt->revision, crypt->encrypt_metadata,
                     crypt->file_key);
    ...
    /* Override: use the raw padded bytes directly in MD5 computation */
    {
        ...
        /* Algorithm 2 step (f) mirror: append 0xFFFFFFFF when metadata is not
         * encrypted and R >= 4. */
        if (!crypt->encrypt_metadata && crypt->revision >= 4) {
            static const uint8_t sentinel[4] = {0xFF, 0xFF, 0xFF, 0xFF};
            md5_update(&md5, sentinel, 4);
        }
...
/* ISO 32000-2 Algorithm 2.B (hardened extended hash, PDF 2.0 / R=6).
 *
 *   K = SHA-256(password + salt [+ udata])
 *   loop:
 *     K1 = (password + K[0:klen] + udata) repeated 64 times
 *     E  = AES-128-CBC-encrypt(K1, key = K[0:16], iv = K[16:32])   (no padding)
 *     hash_id = (128-bit big-endian integer of E[0:16]) mod 3
 *     K = { SHA-256, SHA-384, SHA-512 }[hash_id](E)     klen tracks 32/48/64
 *   until round >= 64 AND E[last] <= round - 32
 *
 * The final file/validation key is K[0:32]. */
static void compute_hash_r6(const uint8_t *password, size_t pw_len,
                            const uint8_t *salt8,
                            const uint8_t *extra, size_t extra_len,
                            uint8_t out[32]) {
    /* K buffer holds up to a 64-byte (SHA-512) digest; klen tracks the live
     * length. It starts at 32 (SHA-256). */
    uint8_t K[64];
    size_t klen;
    {
        Sha256 sha;
        sha256_init(&sha);
        if (pw_len > 0) sha256_update(&sha, password, pw_len);
    ...
    while (1) {
        /* Build K1 = (password + K[0:klen] + extra) repeated 64 times. */
        size_t K1_unit = pw_len + klen + extra_len;
        size_t K1_len = K1_unit * 64;
        uint8_t *K1 = (uint8_t *)malloc(K1_len ? K1_len : 1);
        if (!K1) { memcpy(out, K, 32); return; }

        for (int i = 0; i < 64; i++) {
            size_t off = (size_t)i * K1_unit;
            if (pw_len > 0) memcpy(K1 + off, password, pw_len);
            memcpy(K1 + off + pw_len, K, klen);
            if (extra && extra_len > 0)
                memcpy(K1 + off + pw_len + klen, extra, extra_len);
        }

        /* E = AES-128-CBC(K1), key = K[0:16], iv = K[16:32]. K1_len is always
         * a multiple of 16 (unit*64) so no padding is needed or wanted. */
        uint8_t *E = (uint8_t *)malloc(K1_len ? K1_len : 1);
        ...
        /* hash_id = (first 16 bytes of E as a 128-bit big-endian integer) mod 3.
         * Computed via Horner's method over the bytes: at each step the running
         * remainder is (rem * 256 + byte) mod 3. This is a true mod-3 of the
         * full 128-bit value, not the (incorrect) sum-of-bytes shortcut. */
        unsigned int rem = 0;
        for (int i = 0; i < 16; i++)
            rem = (rem * 256u + E[i]) % 3u;
        int hash_id = (int)rem;
        ...
        if (hash_id == 0) {
            sha256_hash(E, K1_len, K);
            klen = 32;
        } else if (hash_id == 1) {
            sha384_hash(E, K1_len, K);
            klen = 48;
        } else {
            sha512_hash(E, K1_len, K);
            klen = 64;
        }

        uint8_t last_byte = E[K1_len - 1];
        free(E);

        round++;
        /* Exit once we have done at least 64 rounds and the last byte of the
         * most recent E is <= round - 32. */
...
static bool verify_password_v5(TspdfCrypt *crypt,
    ...
    if (memcmp(hash, U, 32) == 0) {
        ...
        /* AES-256-CBC (zero IV, no padding) decrypt UE to get the file key. */
        aes256_cbc_zero_iv_decrypt32(key_hash, UE, crypt->file_key);
    ...
    if (memcmp(hash, O, 32) == 0) {
        ...
        /* AES-256-CBC (zero IV, no padding) decrypt OE to get the file key. */
        aes256_cbc_zero_iv_decrypt32(key_hash, OE, crypt->file_key);
...
TspdfError tspdf_crypt_init(TspdfCrypt *crypt, TspdfObj *encrypt_dict,
    ...
    crypt->use_aes = false;
    /* /EncryptMetadata is optional and defaults to true. */
    crypt->encrypt_metadata = get_bool(encrypt_dict, "EncryptMetadata", true);
    ...
    /* V=4: parse crypt filters. StmF and StrF are resolved independently so a
     * file may use /Identity for one and a real cipher for the other. */
    if (V == 4) {
        const char *stmf = get_name(encrypt_dict, "StmF");
        const char *strf = get_name(encrypt_dict, "StrF");
        /* Both default to /Identity when the key is absent. */
        if (!stmf) stmf = "Identity";
        if (!strf) strf = "Identity";

        TspdfObj *cf = tspdf_dict_get(encrypt_dict, "CF");

        crypt->identity_stm = (strcmp(stmf, "Identity") == 0);
        crypt->identity_str = (strcmp(strf, "Identity") == 0);

        /* Resolve the cipher (use_aes) from whichever purpose actually
         * encrypts. If both do, their ciphers must agree. cfm_out is tri-state:
         * -1 unknown, 0 RC4, 1 AES. */
        int cipher = -1;  /* -1 unknown, 0 = RC4/V2, 1 = AES */
        const char *purposes[2];
        size_t purpose_count = 0;
        if (!crypt->identity_stm) purposes[purpose_count++] = stmf;
        if (!crypt->identity_str) purposes[purpose_count++] = strf;

        for (size_t i = 0; i < purpose_count; i++) {
            const char *filter_name = purposes[i];
            int this_cipher = 0;  /* default V2/RC4 if no CFM present */
            if (cf && cf->type == TSPDF_OBJ_DICT) {
                ...
                if (filt && filt->type == TSPDF_OBJ_DICT) {
                    ...
                    if (cfm) {
                        if (strcmp(cfm, "AESV2") == 0 || strcmp(cfm, "AESV3") == 0) {
                            this_cipher = 1;
                        } else if (strcmp(cfm, "V2") == 0) {
                            this_cipher = 0;
                        } else if (strcmp(cfm, "Identity") == 0) {
                            /* A named filter whose CFM is Identity also means
                             * "do not encrypt" for that purpose. */
                            if (filter_name == stmf) crypt->identity_stm = true;
                            if (filter_name == strf) crypt->identity_str = true;
                            continue;
                        } else {
            ...
            if (cipher == -1) {
                cipher = this_cipher;
            } else if (cipher != this_cipher) {
                /* Streams and strings use different ciphers: not supported. */
                return TSPDF_ERR_UNSUPPORTED;
            }
        }

        crypt->use_aes = (cipher == 1);
    }
    ...
    if (V <= 4) {
        ...
        compute_file_key(password, O_data, O_len, P,
                         ...
                         crypt->key_len, R, crypt->encrypt_metadata,
                         crypt->file_key);
...
TspdfError tspdf_crypt_decrypt_string(TspdfCrypt *crypt, uint32_t obj_num,
    ...
    /* V=4 /StrF /Identity: strings are stored in the clear. */
    if (crypt->identity_str) return TSPDF_OK;
...
uint8_t *tspdf_crypt_decrypt_stream(TspdfCrypt *crypt, uint32_t obj_num,
    ...
    /* V=4 /StmF /Identity: stream data is stored in the clear. Return a copy so
     * the caller can free it uniformly. */
    if (crypt->identity_stm) {
...
TspdfError tspdf_crypt_encrypt_init(TspdfCrypt *crypt, const char *user_pass,
    ...
    crypt->use_aes = true;
    crypt->encrypt_metadata = true;  /* our writer always encrypts metadata */
    ...
    if (key_bits == 128) {
        ...
        crypt->version = 5;
        crypt->revision = 6;   /* R=6, ISO 32000-2: interoperable with qpdf/mutool/Acrobat */
        crypt->key_len = 32;
    } else {
        return TSPDF_ERR_UNSUPPORTED;
    }
    ...
    if (crypt->version == 4) {
        ...
        compute_file_key(user_pass, crypt->O, 32, (int32_t)permissions,
                         crypt->file_id, crypt->file_id_len,
                         crypt->key_len, crypt->revision,
                         crypt->encrypt_metadata, crypt->file_key);
        ...
        /* AES-256 (V=5, R=6, ISO 32000-2). The validation and key-derivation
         * hashes use the extended Algorithm 2.B hash (compute_hash_r6), which is
         * what qpdf, mutool, and Acrobat expect. UE/OE wrap the file key with
        ...
        /* /U = hash_r6(user_pass, validation_salt) + validation_salt + key_salt */
        {
            uint8_t hash[32];
            compute_hash_r6(upw, user_pw_len, u_val_salt, NULL, 0, hash);
        ...
        /* /UE = AES-256-CBC(zero IV) encrypt file_key with hash_r6(user_pass, key_salt) */
        {
            uint8_t key_hash[32];
            compute_hash_r6(upw, user_pw_len, u_key_salt, NULL, 0, key_hash);
            aes256_cbc_zero_iv_encrypt32(key_hash, crypt->file_key, crypt->UE);
        }

        /* /O = hash_r6(owner_pass, validation_salt, /U) + validation_salt + key_salt */
        {
            uint8_t hash[32];
            compute_hash_r6(opw, owner_pw_len, o_val_salt, crypt->U, 48, hash);
        ...
        /* /OE = AES-256-CBC(zero IV) encrypt file_key with hash_r6(owner_pass, key_salt, /U) */
        {
            uint8_t key_hash[32];
            compute_hash_r6(opw, owner_pw_len, o_key_salt, crypt->U, 48, key_hash);
            aes256_cbc_zero_iv_encrypt32(key_hash, crypt->file_key, crypt->OE);
        }

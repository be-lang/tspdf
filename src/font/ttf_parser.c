...
// Upper bound on the number of kerning pairs we will allocate from an untrusted
// font. The kern table can declare up to 65535 subtables of up to 65535 pairs
// each (~4.3 billion), which overflows the pair-count arithmetic and demands an
// absurd allocation. A real font's kern table is tiny by comparison; 1M pairs
// is generous and keeps total_pairs * sizeof(TTF_KernPair) far below INT_MAX.
#define TTF_MAX_KERN_PAIRS 1000000u
...
static const uint8_t *ttf_find_table(const TTF_Font *font, const char *tag_str, uint32_t *out_length) {
    ...
    for (int i = 0; i < font->num_tables; i++) {
        if (font->tables[i].tag == tag) {
            uint32_t off = font->tables[i].offset;
            uint32_t len = font->tables[i].length;
            if (out_length) *out_length = len;
            // Bounds-check the table against the buffer. The offset and length
            // come straight from the (untrusted) sfnt directory, so a malicious
            // length like 0xFFFFFF36 would make `off + len` wrap around in 32-bit
            // arithmetic and slip past a naive `> data_len` test, leaving `off`
            // far outside the buffer. Compare with subtraction against the known
            // size instead so no addition can overflow.
            if (off > font->data_len) return NULL;
            if (len > font->data_len - off) return NULL;
...
static bool parse_kern(TTF_Font *font) {
    ...
    // Count total pairs across all format 0 horizontal subtables. The kern
    // table is untrusted: up to 65535 subtables each declaring up to 65535
    // pairs can sum past INT_MAX, wrapping a signed int negative and making
    // malloc(total_pairs * sizeof(TTF_KernPair)) request a tiny/garbage size
    // while the fill loop still walks to total_pairs (heap overflow). Count in
    // size_t and cap at a sane ceiling so the product can never overflow.
    ...
    for (uint16_t t = 0; t < n_tables; t++) {
        ...
        if (format == 0 && horizontal) {
            ...
            total_pairs += n_pairs;
            if (total_pairs > TTF_MAX_KERN_PAIRS) return false;
        }
        if (sub_length == 0) break;  // avoid a zero-length subtable looping forever
        scan += sub_length;
    }
    ...
    for (uint16_t t = 0; t < n_tables; t++) {
        ...
        if (format == 0 && horizontal) {
            if (offset + 14 > len) { if (sub_length == 0) break; offset += sub_length; continue; }
    ...
    font->kern_pair_count = (int)idx;  // idx <= TTF_MAX_KERN_PAIRS, fits in int
    font->has_kern = true;

    // Sort kern pairs by (left, right) for binary search lookup
    qsort(font->kern_pairs, idx, sizeof(TTF_KernPair), kern_pair_compare);
...
uint16_t ttf_get_glyph_index(const TTF_Font *font, uint32_t codepoint) {
    ...
    if (format == 4) {
        ...
        // Format 6: Trimmed table mapping. The header is 10 bytes (format,
        // length, language, firstCode, entryCount) followed by entryCount*2
        // glyph IDs. cmap_length is the bytes from this subtable to the end of
        // the cmap table and is already bounds-checked against the buffer, so
        // validate every read against it just like format 4/12 do rather than
        // trusting only the table-level offset.
        if (font->cmap_length < 10) return 0;
        uint16_t first_code = read_u16(subtable + 6);
        uint16_t entry_count = read_u16(subtable + 8);
        // glyphIdArray must fit: 10 + entry_count*2 bytes. Use 64-bit math so
        // the product cannot overflow before the comparison.
        if (font->cmap_length < 10 + (uint64_t)entry_count * 2) return 0;
        if (codepoint >= first_code && codepoint < (uint32_t)first_code + entry_count) {
